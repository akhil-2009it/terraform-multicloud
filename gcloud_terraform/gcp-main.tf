...
resource "google_compute_network" "gcp-vpc" {
  name                    = "${var.app-name}-${var.app-environment}-vpc"
...
resource "google_compute_subnetwork" "gcp-subnet" {
  name          = "${var.app-name}-${var.app-environment}-subnet"
...
resource "google_compute_firewall" "gcp-allow-http" {
  name    = "${var.app-name}-${var.app-environment}-fw-allow-http"
  network = google_compute_network.gcp-vpc.name
  allow {
    protocol = "tcp"
    ports    = ["80"]
...
resource "google_compute_firewall" "gcp-allow-ssh" {
  name    = "${var.app-name}-${var.app-environment}-fw-allow-ssh"
  network = google_compute_network.gcp-vpc.name
  allow {
    protocol = "tcp"
    ports    = ["22"]
...
resource "google_compute_address" "gcp-web-ip" {
  name    = "${var.app-name}-${var.app-environment}-web-ip"
  project = var.gcp_project
  region  = var.gcp_region
}
...
resource "google_compute_instance" "gpc-web-server" {
  name         = "${var.app-name}-${var.app-environment}-web-server"
